...
현재 branch를 비교 기준으로 붙이면 같은 루프가 `MonsterGASAnimInstance -> BTTask_TraceGAS / BTTask_AttackGAS -> AnimNotify -> Ability.Attack / GameplayEffect_Damage` 쪽으로 다시 이어진다.
...
또한 이번 보강에서는 Epic Developer Community의 언리얼 공식 문서를 함께 참조해, `Behavior Tree`, `Anim Notify`, `Navigation` 용어를 엔진 표준 문법으로 다시 연결한다.
...
- 기존 `TakeDamage` 직통 루프와 현재 `Ability.Attack -> GameplayEffect_Damage -> GameplayCue.Battle.Attack` 루프 차이를 설명할 수 있다.
- `Behavior Tree`, `Basic Navigation`, `Animation Blueprint`, `Animation Notifies` 공식 문서가 왜 `260416`과 직접 연결되는지 설명할 수 있다.
...
5. 언리얼 공식 문서를 통해 `Behavior Tree`, `Navigation`, `Animation Notify`가 엔진 표준 용어로 어떻게 정리되는지 확인한다.
6. 현재 프로젝트 C++ 코드를 읽으며, 위 구조가 `MonsterAnimInstance`, `MonsterGASAnimInstance`, `BTTask_MonsterTrace`, `BTTask_TraceGAS`, `BTTask_AttackGAS`, `MonsterNormal`, `MonsterNormalGAS_Warrior` 안에서 어떻게 하나의 전투 루프로 이어지는지 확인한다.

## 2026-04-24 최신 branch 비교 메모

이번 보강에서는 몬스터 공격 루프를 `AnimInstance`, `BTTask_MonsterAttack`, 자식 몬스터 자산 덤프뿐 아니라 현재 `Monster/GAS` 전투 코드와 함께 다시 맞춰 봤다. 그래서 이제 이 날짜는 "공격 애니메이션을 붙인다"가 아니라 "애니메이션 노티파이와 블랙보드가 서로 신호를 주고받는 루프를 만든다"로 읽는 편이 정확하다.

- `MonsterAnimInstance_FileDump.txt`를 보면 이 인스턴스는 `mAnimMap`, `mBlendSpaceMap`, `mMontageMap`, `mHitAlpha`를 들고 있고, `AnimNotify_Attack()`에서 `Monster->NormalAttack()`, `AnimNotify_AttackEnd()`에서 블랙보드 `AttackEnd = true`, `AnimNotify_Death()`에서 `Monster->Death()`를 호출한다. 즉 노티파이는 단순 연출 포인트가 아니라 게임플레이 전환 지점이다.
- `BTTask_MonsterAttack_FileDump.txt`는 공격 태스크가 시작할 때 `Monster->ChangeAnim(EMonsterNormalAnim::Attack)`를 호출하고, 블랙보드 `AttackTarget`에 현재 타깃을 써 넣은 다음, 실행 중에는 `AttackEnd`가 켜질 때까지 기다렸다가 다시 `false`로 되돌린다는 사실을 보여 준다. 그래서 강의에서 설명한 `AttackTarget`과 `AttackEnd`의 분업이 실제 구현에서도 그대로 보인다.
- `MonsterNormal_FileDump.txt`는 `NormalAttack()`가 블랙보드에 저장된 `AttackTarget`을 읽어 직접 데미지를 적용하는 흐름을 보여 준다. 즉 추적 태스크는 거리를 맞추고, 공격 태스크는 상태를 `Attack`으로 바꾸고, 실제 타격은 애니메이션 노티파이 프레임에 실행된다.
- `ABP_MonsterNormal_NodeDump.txt`, `ABP_Monster_MinionWarrior_NodeDump.txt`, `ABP_Monster_MinionGunner_NodeDump.txt`를 보면 워리어와 거너 애님 블루프린트는 각각 공용 `ABP_MonsterNormal`을 상속한 자식 자산이다. 둘 다 별도 상태머신을 새로 만드는 대신 공용 로직을 재사용하므로, 이번 강의의 핵심은 "자산을 갈아끼우는 방식의 몬스터 확장"에 가깝다.
- `MonsterNormal_MinionWarrior_FileDump.txt`, `MonsterNormal_MinionGunner_FileDump.txt`, `Attack_A_AssetDump.txt`, `Fire_A_AssetDump.txt`, `P_Minion_Impact_Default_AssetDump.txt`를 보면 워리어는 근접 메시와 `ABP_Monster_MinionWarrior`, 거너는 원거리 메시와 `ABP_Monster_MinionGunner`를 쓰고, 대표 공격 시퀀스는 둘 다 약 `1.0초` 길이이며 타격 이펙트는 동일한 `P_Minion_Impact_Default`를 공유한다. 즉 둘의 차이는 전투 구조보다 메시/애님/데이터셋 선택에 더 가깝다.
- 현재 `MonsterGASAnimInstance.cpp`도 `AnimNotify_Attack()`, `AnimNotify_AttackEnd()`, `AnimNotify_Death()` 계약을 거의 그대로 유지한다. 즉 현재 branch에서도 공격 루프의 시간축은 여전히 "애니메이션 노티파이 -> 블랙보드 플래그 / 본체 함수"로 이어진다.
- `BTTask_AttackGAS.cpp`는 `AttackTarget`, `AttackEnd` 사용법을 그대로 유지하면서, 공격 거리 재평가를 `Monster->GetAttributeSet()->GetAttackDistance()`로 바꿔 읽는다. 즉 루프의 형태는 같고, 수치 공급원만 `MonsterState` 멤버에서 `AttributeSet`으로 이동했다.
- `MonsterNormalGAS_Warrior.cpp`는 `NormalAttack()`에서 더 이상 `TakeDamage()`를 직접 부르지 않고, `Ability.Attack` 태그가 달린 `FGameplayEventData`를 자신에게 보낸다. 그리고 `UGameplayAbility_Attack`가 `Attack - Defense`를 계산해 `GameplayEffect_Damage`를 적용하고, `GameplayCue.Battle.Attack`가 이펙트를 처리한다.
- 반대로 `MonsterNormalGAS_Gunner.cpp`는 아직 `Ability.Attack` 이벤트를 보내지 않고 파티클만 뿌린다. 즉 현재 branch의 전투 GAS 이관은 워리어 쪽이 먼저 진행됐고, 거너 쪽은 아직 연출 중심 상태라고 보는 편이 맞다.
...
// 실제 타격 대상은 AttackTarget 슬롯에 따로 저장
...
{
    // 정확한 타격 프레임에서만 실제 공격 함수를 호출한다.
    Monster->NormalAttack();
}

void UMonsterAnimInstance::AnimNotify_AttackEnd()
{
    // 공격이 끝났다는 사실을 블랙보드 플래그로 AI 태스크에 전달한다.
...
현재 branch 비교 메모를 붙이면, 이 "실제 데미지를 누가 주는가"라는 질문의 답이 한 단계 더 확장된다.
`AMonsterNormalGAS_Warrior::NormalAttack()`는 더 이상 `TakeDamage()`를 직접 호출하지 않고, `AttackTarget`을 읽어 `Ability.Attack` 이벤트를 자기 자신에게 보낸다.
...
그리고 `UGameplayAbility_Attack::ActivateAbility()`가 이 이벤트를 받아 `SourceASC`와 `TargetASC`의 `AttributeSet`에서 `Attack`, `Defense`를 읽고,
`UGameplayEffect_Damage`에 `Effect.Battle.Damage` 값을 넣어 적용한다.
이 이펙트는 다시 `GameplayCue.Battle.Attack`를 발생시켜 타격 이펙트와 사운드까지 연결한다.

즉 `260416`의 공격 루프는 현재 branch에서도 여전히 `AttackTask -> AnimNotify_Attack() -> NormalAttack()` 순서를 유지하지만,
"실제 데미지를 계산하고 HP에 반영하는 층"은 `Monster` 본체의 직접 `TakeDamage()` 호출에서 `GAS Ability / Effect / Cue` 계층으로 이동한 셈이다.
...
## 제4장. 언리얼 공식 문서로 다시 읽는 260416 핵심 구조

### 4.1 왜 260416부터 공식 문서를 같이 보는가

`260416`은 몬스터가 실제 전투 루프를 도는 날이다.
입문자 입장에서는 `Trace Task`, `Attack Task`, `AnimNotify`, `NormalAttack()`이 각각 따로 노는 기능처럼 보일 수 있지만, 공식 문서 기준으로 보면 이들은 `Behavior Tree`, `Navigation`, `Animation Blueprint`, `Animation Notifies` 층으로 정리된다.
...
즉 이 장의 목적은 `Trace -> Attack -> Notify` 흐름이 언리얼 공식 문서 기준으로 어떤 시스템 층에 해당하는지 보여 주는 데 있다.

### 4.2 공식 문서의 `Behavior Tree`와 `Navigation`은 강의의 `Trace -> Attack` 전이를 더 선명하게 만든다

강의 2장과 3장의 핵심은 몬스터가 언제까지 추적하고, 언제부터 공격 문맥으로 넘어갈지를 정하는 것이다.
공식 문서 기준으로 보면 이 과정은 아래처럼 읽을 수 있다.

- `Navigation`: 지금 타깃에게 실제로 이동할 수 있는가
- `Behavior Tree`: 이동 브랜치를 계속 탈지, 다음 행동 브랜치로 넘어갈지 결정하는가

즉 `BTTask_MonsterTrace`가 공격 거리 진입 시 `Failed`를 반환하는 구조도 "오류"라기보다, 트리 안에서 다음 행동 브랜치를 열기 위한 전이 규칙으로 이해할 수 있다.

### 4.3 공식 문서의 `Animation Blueprint`와 `Animation Notifies`는 강의의 `AttackTarget / AttackEnd` 설계를 시간축 관점으로 정리해 준다

강의 1장과 3장은 몬스터 애니메이션을 입력 기반이 아니라 상태 기반으로 다룬다.
공식 문서의 `Animation Blueprint`와 `Animation Notifies`도 같은 점을 강조한다.

- 애니메이션 상태는 `AnimInstance`가 보관한다
- 실제 타격 시점은 `Notify`가 알려 준다
- 공격 종료 시점도 별도 신호로 다룰 수 있다

즉 `260416`의 중요한 포인트는 공격 버튼이 없다는 점이다.
플레이어 입력 대신, AI 태스크가 공격 문맥을 세팅하고, 애니메이션이 재생되다가, 정확한 프레임에 `AnimNotify_Attack()`가 올라오고, 그때 실제 데미지가 적용된다.
그래서 이 날짜는 기능 추가보다 `전투 타이밍을 시스템끼리 어떻게 연결하는가`를 배우는 날이라고 보는 편이 맞다.

### 4.4 260416 공식 문서 추천 읽기 순서
...
이 순서가 좋은 이유는 먼저 `AI가 어떤 브랜치를 타는가`를 BT와 Navigation으로 잡고, 그 다음 `그 행동이 애니메이션 시간축과 어떻게 연결되는가`를 애님 문서로 내려가 읽을 수 있기 때문이다.
...
## 제5장. 현재 프로젝트 C++ 코드로 다시 읽는 260416 핵심 구조

### 5.1 왜 260416은 "공격 기능 추가"보다 "전투 타이밍 연결" 강의인가
...
현재 저장소에는 이 legacy 루프와 GAS 루프가 공존한다.
둘 다 `AttackTarget`, `AttackEnd`, `AnimNotify` 골격은 같지만, 마지막 데미지 전달 지점만 다르다.
legacy는 `AMonsterNormal::NormalAttack() -> Target->TakeDamage()`로 닫히고,
GAS 워리어는 `AMonsterNormalGAS_Warrior::NormalAttack() -> GameplayAbility_Attack -> GameplayEffect_Damage`로 닫힌다.
...
### 5.2 `UMonsterAnimInstance`: 몬스터 애니메이션 상태와 노티파이 이벤트를 들고 있는 얇은 다리
...
현재 `UMonsterGASAnimInstance`도 멤버 구성과 노티파이 계약이 거의 같다.
즉 이 장의 핵심은 특정 클래스명을 외우는 데 있기보다, "상태 저장 + 노티파이 전달"이라는 계약이 legacy와 GAS 양쪽에서 유지된다는 점을 보는 데 있다.

### 5.3 `AnimNotify_Attack()`, `AnimNotify_AttackEnd()`, `AnimNotify_Death()`: 애니메이션 프레임을 게임 규칙으로 번역한다
...
### 5.4 `UBTTask_MonsterTrace`: 공격 태스크를 열기 전까지 계속 타겟을 좇는 장기 태스크
...
현재 `UBTTask_TraceGAS`도 구조는 거의 같다.
다만 공격 거리 판정은 `Monster->GetAttributeSet()->GetAttackDistance()`를 읽는다.
즉 `260416`의 전이 규칙은 유지되되, 거리 수치의 공급원만 `MonsterState` 멤버에서 `AttributeSet`으로 이동했다.

### 5.5 `UBTTask_MonsterAttack`: 데미지를 바로 주지 않고 "공격 문맥"을 세팅한 뒤 노티파이를 기다린다
...
### 5.6 `BTTask_MonsterAttack::TickTask()`: 공격 종료 신호가 올라온 뒤에만 다음 행동을 고른다
...
현재 `UBTTask_AttackGAS`도 이 구조를 거의 그대로 유지한다.
차이는 공격 종료 후 거리 재평가에 `Monster->GetAttributeSet()->GetAttackDistance()`를 쓰고, 정리 단계에서도 여전히 `AttackTarget = nullptr`로 공격 문맥을 닫는다는 점이다.

### 5.7 `AMonsterNormal::NormalAttack()`: 실제 데미지는 몬스터 본체가 `AttackTarget`을 읽어 처리한다
...
### 5.8 현재 branch 비교: Warrior는 GAS 데미지로 넘어갔고, Gunner는 아직 연출 중심이다

강의 원형의 `MonsterNormal_MinionWarrior`, `MonsterNormal_MinionGunner`는 둘 다 부모의 직접 데미지 루프를 재사용하고 뒤에 이펙트를 덧붙이는 구조였다.
하지만 현재 `Monster/GAS` 라인을 보면 전사형은 이미 GAS 공격 루프로 넘어갔다.
...
이 이벤트는 `UGameplayAbility_Attack::ActivateAbility()`로 넘어가,
`SourceASC`와 `TargetASC`의 `AttributeSet`에서 `Attack`, `Defense`를 읽고,
`Attack - Defense`를 `Effect.Battle.Damage` 값으로 넣은 `UGameplayEffect_Damage`를 적용한다.
그 결과 `GameplayCue.Battle.Attack`까지 이어져 타격 이펙트와 사운드도 같이 발생한다.

즉 현재 워리어의 실제 전투 파이프라인은 다음처럼 읽는 편이 맞다.

`Trace -> AttackTarget -> AnimNotify_Attack -> Ability.Attack 이벤트 -> GameplayAbility_Attack -> GameplayEffect_Damage -> GameplayCue.Battle.Attack`

반대로 `AMonsterNormalGAS_Gunner::NormalAttack()`는 아직 `Ability.Attack` 이벤트를 보내지 않고, `AttackTarget` 위치에 파티클만 뿌린다.
즉 현재 branch의 공격 GAS 이관은 워리어 쪽이 먼저 진행됐고, 거너 쪽은 아직 "애니메이션/이펙트는 붙었지만 데미지 백엔드는 미이관" 상태라고 보는 편이 정확하다.

이 차이는 학습용으로 오히려 유용하다.
같은 `AttackTarget`, `AttackEnd`, `AnimNotify` 뼈대를 유지한 채, 마지막 데미지 전달 계층만 `TakeDamage`에서 `Ability / Effect / Cue`로 갈아끼우는 과정을 한 프로젝트 안에서 비교해 볼 수 있기 때문이다.

### 5.9 `AMonsterBase::TakeDamage()`와 `Death()`: 전투 루프의 끝도 몬스터 본체가 책임진다
...
현재 `AMonsterGAS::TakeDamage()`를 같이 보면, 예전의 수동 HP 차감 로직은 주석 처리돼 있고 `Death()` 쪽 랙돌/수명 정리 코드는 여전히 남아 있다.
즉 현재 branch에서는 "HP를 깎는 층"이 `Pawn::TakeDamage()`에서 `AttributeSet / GameplayEffect` 쪽으로 이동하고,
"죽은 뒤 몸체를 물리적으로 정리하는 층"은 여전히 몬스터 본체가 맡는 쪽으로 역할이 더 분리된 셈이다.
...
그리고 현재 branch 비교까지 붙이면, 이 계층 분리가 `TakeDamage` 중심 루프에서 `Ability / Effect / Cue` 중심 루프로 확장되는 과정도 함께 읽을 수 있다.
...
6. 현재 branch에서는 이 마지막 데미지 전달 층이 워리어 기준 `Ability.Attack -> GameplayAbility_Attack -> GameplayEffect_Damage -> GameplayCue.Battle.Attack`로 옮겨 가고, 거너는 아직 연출 중심 상태에 머물러 있다.
7. 공식 문서 기준으로는 이 흐름이 `Behavior Tree + Navigation + Animation Blueprint + Animation Notifies` 조합으로 설명되고, 현재 C++ 기준으로는 `Trace -> AttackTarget 세팅 -> AnimNotify_Attack -> NormalAttack -> TakeDamage -> AnimNotify_Death`까지가 하나의 종단 간 전투 파이프라인이다.
...
- `Behavior Tree`, `Navigation`, `Animation Blueprint`, `Animation Notifies` 문서가 어느 파트를 보강하는지 연결할 수 있는가
- `AnimNotify_Attack() -> AMonsterNormal::NormalAttack() -> Target->TakeDamage()` 호출 흐름을 코드 기준으로 설명할 수 있는가
- `AnimNotify_Attack() -> AMonsterNormalGAS_Warrior::NormalAttack() -> GameplayAbility_Attack -> GameplayEffect_Damage()` 호출 흐름을 legacy 루프와 비교해 설명할 수 있는가
...
4. 공식 문서의 `Behavior Tree / Navigation / Animation Notify` 구도를 먼저 알고 강의를 읽으면 무엇이 더 덜 헷갈릴까.
...
4. 공식 문서의 `Behavior Tree User Guide`, `Animation Blueprint`, `Animation Notifies`를 읽고 강의 구조와 어떤 용어가 대응되는지 표로 정리해 본다.
