...
6. 현재 프로젝트 C++ 코드를 읽으며, 위 구조가 `MonsterAnimInstance`, `BTTask_MonsterTrace`, `BTTask_MonsterAttack`, `MonsterNormal` 안에서 어떻게 하나의 전투 루프로 이어지는지 확인한다.

## 2026-04-23 덤프 반영 메모

이번 보강에서는 몬스터 공격 루프를 `AnimInstance`, `BTTask_MonsterAttack`, 자식 몬스터 자산 덤프로 다시 맞춰 봤다. 그래서 이제 이 날짜는 "공격 애니메이션을 붙인다"가 아니라 "애니메이션 노티파이와 블랙보드가 서로 신호를 주고받는 루프를 만든다"로 읽는 편이 정확하다.
...
### 5.8 `MonsterNormal_MinionWarrior`, `MonsterNormal_MinionGunner`: 현재 자식 클래스는 공격 방식보다 연출을 덧붙이는 쪽에 가깝다
...
6. 공식 문서 기준으로는 이 흐름이 `Behavior Tree + Navigation + Animation Blueprint + Animation Notifies` 조합으로 설명되고, 현재 C++ 기준으로는 `Trace -> AttackTarget 세팅 -> AnimNotify_Attack -> NormalAttack -> TakeDamage -> AnimNotify_Death`까지가 하나의 종단 간 전투 파이프라인이다.
