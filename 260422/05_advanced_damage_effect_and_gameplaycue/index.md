...
[이전: 고급 1편](../04_advanced_targetdata_and_damage_preview/) | [허브](../) | [다음: 260423 중급 1편](../../260423/01_intermediate_gas_attack_completion/)
...
강의 화면에서는 `DamageSpec` 생성, `Attack - Defense` 계산, `FMath::Max(1.f, Damage)`, `Effect.Battle.Damage` 주입이 한 구간에 연속으로 나온다.
그래서 현재 프로젝트가 `ExecutionCalculation` 없이도 "계산값을 Effect에 실어 타겟에게 적용한다"는 구조를 훨씬 직접적으로 보여 준다.

![DamageSpec에 Effect.Battle.Damage 값을 주입하는 장면](../assets/images/damage-spec-setbycaller.jpg)
...
이 흐름이 실제 `GameplayCue` 자산 연결과 `MonsterGAS` 전투 루프까지 어떻게 확장되는지는
[260423 교재](../../260423/)에서 이어서 읽을 수 있다.
