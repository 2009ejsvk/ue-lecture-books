...
- `Collision Overview`, `Using Gameplay Tags`, `Variables, Timers, and Events`, `Trigger Volume Actors`, `Actors Reference` 공식 문서가 왜 `260403`과 직접 연결되는지 설명할 수 있다.
...
4. 언리얼 공식 문서를 통해 충돌 응답, 태그, 타이머, 트리거 볼륨이 엔진 표준 용어로 어떻게 정리되는지 확인한다.
5. 현재 프로젝트의 C++ 코드를 읽으며, 위 구조가 `ProjectileBase`, `WraithBullet`, `GeometryActor`, `ItemBox`, `MonsterBase`, `MonsterSpawnPoint` 안에서 어떤 식으로 정리되었는지 확인한다.
...
```cpp
// 공통 데미지 루트를 거쳐 방어력만큼 빼고 HP에 반영한다.
...
## 제4장. 언리얼 공식 문서로 다시 읽는 260403 핵심 구조

### 4.1 왜 260403부터 공식 문서를 같이 보는가

`260403`은 충돌, 태그, 타이머, 트리거라는 네 가지 기초 도구가 한꺼번에 등장하는 날이다.
입문자 입장에서는 전부 블루프린트 노드처럼 보이지만, 언리얼 공식 문서에서는 이 개념들을 각각 `Collision`, `Gameplay Tags`, `Timers`, `Trigger Volumes`처럼 엔진 공통 규칙으로 정리한다.
...
즉 이 장의 목적은 강의 내용을 다른 방식으로 다시 설명하는 것이 아니라, `260403`에서 처음 본 기능들이 언리얼 표준 문서에서는 어떤 카테고리와 용어로 묶이는지 보여 주는 데 있다.

### 4.2 충돌 공식 문서는 `Block / Overlap / Ignore`를 "반응 규칙표"로 정리한다

강의 1장에서 가장 먼저 배우는 것은 총알이 무엇에 막히고, 무엇과 겹치고, 무엇을 무시할지 정하는 규칙이다.
공식 문서의 `Collision Overview`도 정확히 같은 관점으로 시작한다.

즉 `260403`의 핵심은 "총알이 부딪혔다"가 아니라 아래 세 가지를 설계하는 데 있다.

- 무엇과는 막힐 것인가
- 무엇과는 겹치기만 할 것인가
- 무엇은 완전히 무시할 것인가

이 관점으로 보면 강의에서 `Projectile Stop`, `Hit`, `Overlap`을 나눠 설명한 이유도 분명해진다.
공식 문서는 먼저 충돌 응답 규칙을 세우고, 그 위에서 각 이벤트 진입점을 선택하라고 안내하고 있고, 이번 강의도 정확히 그 순서로 가르친다.

### 4.3 공식 문서의 `Gameplay Tags`는 강의의 `PlayerBullet / MonsterBullet` 예제를 더 큰 설계 문법으로 확장해 준다

강의 2장에서 태그는 아주 가볍고 직관적인 식별자로 소개된다.
`PlayerBullet`, `MonsterBullet` 같은 문자열만 붙여도 같은 발사체 클래스 안에서 소속을 구분할 수 있기 때문이다.

공식 문서의 `Using Gameplay Tags`는 이 아이디어를 더 일반화한다.
즉 태그는 단순한 이름표가 아니라, 오브젝트의 정체성, 상태, 규칙 분기를 계층적으로 표현할 수 있는 분류 체계다.

물론 현재 강의와 프로젝트는 아직 정식 `GameplayTag` 시스템까지 가지 않고, 더 가벼운 `Actor Tag` 단계에 머물러 있다.
하지만 초보자 입장에서는 이 연결이 중요하다.

- 강의의 `Actor Tag`: 가장 빠른 입문용 식별 방식
- 공식 문서의 `Gameplay Tags`: 프로젝트가 커졌을 때 쓰는 확장형 식별 방식

즉 `260403`은 태그의 최종형을 배우는 날이 아니라, "식별 규칙을 오브젝트에 붙인다"는 사고방식을 처음 익히는 날이라고 보면 된다.

### 4.4 공식 문서의 타이머 설명은 `Set Timer by Event`를 "주기 실행 도구"로 다시 정리해 준다

강의 2장에서 몬스터는 `Set Timer by Event`로 일정 주기마다 총알을 발사한다.
블루프린트 실습에서는 이 노드가 마치 "주기 발사 기능"처럼 느껴지지만, 공식 문서의 `Variables, Timers, and Events`는 이를 더 일반적인 문법으로 설명한다.

타이머의 핵심은 아래 두 가지다.

- 지금 당장 실행하지 않고 나중에 실행한다
- 한 번만 실행할 수도 있고 반복 실행할 수도 있다

이 관점으로 보면 강의의 `Fire` 이벤트도 단순 총알 함수가 아니다.
시간이 흐를 때마다 다시 호출될 수 있는 행동 단위이고, 나중에는 문 열기, 함정 발동, 버프 종료 같은 다른 시스템에도 그대로 확장될 수 있다.

즉 `260403`의 타이머 실습은 "몹이 총알을 쏜다"보다 먼저, `시간 기반 이벤트를 시스템으로 다루는 법`을 배우는 입문 단계다.

### 4.5 `Trigger Volume Actors`와 `Actors Reference`는 강의의 `Trigger Box + Level Blueprint`를 맵 규칙 설계로 묶어 준다

강의 3장은 `Trigger Box`와 `Level Blueprint`를 이용해 함정을 만드는 파트다.
공식 문서의 `Trigger Volume Actors`와 `Actors Reference`는 이 실습을 더 넓은 관점으로 정리해 준다.

문서 기준으로 트리거는 "플레이어가 들어오면 이벤트를 발생시키는 특수 볼륨 액터"다.
즉 이것은 전투 전용 기능이 아니라, 아래 같은 모든 맵 연출의 출발점이 될 수 있다.
...
그래서 강의에서 `Trigger Box`를 `Block`이 아니라 `Overlap`으로 설명한 것은 아주 정석적이다.
트리거의 목적은 막는 것이 아니라, 월드 안에서 "어떤 조건이 성립했다"는 사실을 이벤트로 바꾸는 데 있기 때문이다.

### 4.6 260403 공식 문서 추천 읽기 순서
...
2. [Trigger Volume Actors in Unreal Engine](https://dev.epicgames.com/documentation/en-us/unreal-engine/trigger-volume-actors-in-unreal-engine?application_version=5.6)
3. [Unreal Engine Actors Reference](https://dev.epicgames.com/documentation/en-us/unreal-engine/unreal-engine-actors-reference)
4. [Quick Start Guide to Variables, Timers, and Events in Unreal Engine C++](https://dev.epicgames.com/documentation/en-us/unreal-engine/quick-start-guide-to-variables-timers-and-events-in-unreal-engine-cpp?application_version=5.6)
5. [Using Gameplay Tags in Unreal Engine](https://dev.epicgames.com/documentation/en-us/unreal-engine/using-gameplay-tags-in-unreal-engine)

이 순서가 좋은 이유는 먼저 `충돌과 트리거`로 이벤트 입구를 이해하고, 그 다음 `타이머`로 시간 제어를 붙이고, 마지막에 `태그`로 대상을 식별하는 사고를 확장할 수 있기 때문이다.

### 4.7 장 정리

공식 문서 기준으로 다시 보면 `260403`은 아래 네 가지를 배우는 날이다.

1. 충돌은 물리 반응이 아니라 규칙표다.
2. 태그는 반응 대상의 소속과 정체성을 구분하는 첫 단계다.
3. 타이머는 반복 행동을 만드는 일반 문법이다.
4. 트리거는 충돌 기반 규칙을 맵 기믹으로 확장하는 장치다.

즉 `260403`은 초보자용 블루프린트 예제를 넘어, 이후 전투와 AI와 퍼즐이 공통으로 기대는 언리얼 시스템 기초를 한 번에 배우는 날이라고 볼 수 있다.

---

## 제5장. 현재 프로젝트 C++ 코드로 다시 읽는 260403 핵심 구조

### 5.1 왜 260403은 C++ 보충을 할 때 "완전히 같은 코드"보다 "같은 원리"를 찾아야 하는가
...
### 5.2 `AProjectileBase`와 `AWraithBullet`: `Projectile Stop`, `Hit`, 충돌 프로파일이 코드에서 만나는 방식
...
### 5.3 `Hit Result`: 맞았다는 사실만이 아니라, 어디를 어떻게 맞았는지를 읽는 정보 묶음
...
### 5.4 태그, 충돌 프로파일, 팀 ID, `TakeDamage`: 소속 구분이 프로젝트 안에서 어떻게 성장하는가
...
{
    // 먼저 언리얼 공통 데미지 진입점을 한 번 통과한다.
    float Dmg = Super::TakeDamage(DamageAmount, DamageEvent, EventInstigator, DamageCauser);

    // 방어력을 뺀 값을 실제 피해량으로 사용한다.
    Dmg -= mDefense;
    // 너무 작은 값이 나오더라도 최소 1은 깎이게 보정한다.
    if (Dmg < 1.f)
        Dmg = 1.f;
...
### 5.5 `Overlap`과 트리거: `Level Blueprint` 실습은 나중에 액터 내부 이벤트로 옮겨 간다
...
### 5.6 C++ 타이머 문법: 정확한 몬스터 발사 예제는 BP 덤프에 있고, 소스에는 타이머 API가 남아 있다
...
그리고 이번 정리본에서는 여기에 더해, 언리얼 공식 문서 기준으로 충돌, 태그, 타이머, 트리거가 어떤 표준 용어로 묶이는지도 먼저 확인하고, 그 다음 현재 `UE20252` C++ 소스에서 `ProjectileBase`, `WraithBullet`, `GeometryActor`, `ItemBox`, `MonsterBase` 같은 클래스로 어떻게 정리되었는지도 함께 읽는다.
...
- `Collision Overview`, `Gameplay Tags`, `Variables, Timers, and Events`, `Trigger Volume Actors` 중 어떤 문서가 각 파트를 보강하는지 연결할 수 있는가?
...
4. 공식 문서의 `Gameplay Tags` 체계와 강의의 `Actor Tag` 예제는 어디까지 같고 어디서부터 달라질까?
5. 태그 기반 판정이 현재 프로젝트에서 충돌 프로파일과 팀 ID, `TakeDamage()` 구조로 발전한 이유는 무엇일까?
...
4. 공식 문서의 `Collision Overview`, `Trigger Volume Actors`, `Variables, Timers, and Events`를 읽고, 강의 예제와 같은 개념이 어떤 용어로 설명되는지 5문장 안으로 비교해 본다.
5. `AProjectileBase`, `AWraithBullet`, `AItemBox`를 보고 각각이 왜 `Projectile Stop`, `Hit`, `Overlap` 중 다른 입구를 선택했는지 스스로 비교해 본다.
