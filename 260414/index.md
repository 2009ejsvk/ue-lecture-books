...
## 제5장. 현재 프로젝트 C++ 코드로 다시 읽는 260414 핵심 구조

### 5.1 왜 260414는 한 클래스만 보면 이해가 끝나지 않는가
...
### 5.2 `AMonsterBase`: Pawn 위에 몸체, 메시, 이동, 팀, 데이터 이름을 얹는다
...
### 5.3 생성자와 `BeginPlay()`: 몸체를 만들고, AI와 데이터 로딩의 기본 전제를 깐다
...
### 5.4 `IMonsterState`와 `FMonsterInfo`: 런타임 상태와 데이터 원본을 분리한다
...
### 5.5 `MonsterInfoLoadComplete()`: 데이터가 실제 몬스터로 주입되는 순간
...
### 5.6 `AMonsterController`: 감각 설정, 팀 판정, 블랙보드 갱신을 담당하는 AI 두뇌
...
### 5.7 `OnTarget()`: Perception 이벤트를 블랙보드와 몬스터 반응으로 번역한다
...
### 5.8 `AMonsterNormal::PossessedBy()`: 빙의 순간에 비헤이비어 트리를 실제로 붙인다
...
### 5.9 `UAssetGameInstanceSubsystem`과 `UMonsterInfoTableAsset`: 데이터 테이블을 중앙에서 비동기로 공급한다
...
### 5.10 파생 몬스터 클래스: 공통 몸체는 재사용하고, 외형과 데이터 이름만 바꾼다
...
5. 실제 C++에서는 `MonsterNormal::PossessedBy()`가 BT 실행을 연결하고, `MonsterBase::BeginPlay()`와 `MonsterInfoLoadComplete()`가 데이터 주입과 AI 반응값 적용을 마무리한다.
...
5. `OnTarget()`이 블랙보드 값만 바꾸는 구조와, `DetectTarget()`까지 함께 호출하는 구조는 어떤 차이를 만들까
...
5. `MonsterBase.cpp`, `MonsterController.cpp`, `MonsterNormal.cpp`, `AssetGameInstanceSubsystem.cpp`를 나란히 열어 "스폰 -> 빙의 -> BT 실행 -> 데이터 로드 -> 감지 반응" 흐름을 화살표 다이어그램으로 정리해 본다.
