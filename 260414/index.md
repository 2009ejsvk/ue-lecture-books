...
`몬스터 몸체 준비 -> 적 감지 -> 행동 고르기 -> 능력치 읽기`

코드 이름으로 다시 쓰면 아래 흐름이다.

`MonsterBase -> AIController / Perception -> Behavior Tree / Blackboard -> DataTable -> AssetManager`

현재 branch를 비교 기준으로 붙이면 같은 뼈대가 `MonsterGAS -> MonsterGASController / ASC / AttributeSet -> Behavior Tree / Blackboard -> DataTable -> AssetManager`로 다시 나타난다.
...
- 강의의 `MonsterState` 중심 설계가 현재 branch에서 `UBaseAttributeSet / UMonsterAttributeSet`과 `AbilitySystemComponent` 쪽으로 어떻게 옮겨 갔는지 설명할 수 있다.
...
6. 현재 프로젝트 C++ 코드를 읽으며, 위 구조가 `MonsterBase`, `MonsterGAS`, `MonsterController`, `MonsterGASController`, `AssetGameInstanceSubsystem` 안에서 어떻게 하나의 AI 기반으로 이어지는지 확인한다.

## 2026-04-24 최신 branch 비교 메모

이번 보강에서는 `Saved\AcademyUtility`에 새로 추가된 몬스터 계열 덤프와 현재 `Monster/GAS` 소스를 함께 대조해, 이 날짜의 설명을 "개념 소개"가 아니라 `UE_Academy_Stduy` 실제 구현 기준으로 다시 고정했다.

- `DumpPath_Summary.txt` 기준 이번 몬스터 덤프 실행은 `RequestedPathCount: 52`, `Dumped=52`, `Missing=0`, `Failed=0`으로 끝났다. 즉 `260414` 본문에서 다루는 데이터/AI 축은 지금 덤프 결과만으로도 거의 전부 역추적할 수 있다.
- `DefaultGame_FileDump.txt`, `PDA_MonsterInfo_AssetDump.txt`, `DT_MonsterInfo_AssetDump.txt`를 보면 `PrimaryAssetType="MonsterInfoTableAsset"`가 `/Game/Monster`를 스캔하고, `PDA_MonsterInfo`는 실제로 `DT_MonsterInfo`를 가리킨다. 또한 `MinionWarrior`와 `MinionGunner` 두 행에 공격력, 방어력, HP/MP, 이동 속도, 감지 거리, 공격 거리, 골드가 모두 정리돼 있어 "몬스터 능력치는 DataTable에서 읽는다"는 강의 설명이 정확히 맞다.
- `BB_Monster_Base_AssetDump.txt`와 `BB_Monster_Normal_AssetDump.txt`를 보면 실사용 블랙보드 키는 `SelfActor`, `DetectRange`, `Target`, `AttackTarget`, `AttackEnd`, `WaitTime` 6개이고, `BB_Monster_Normal`은 여기에 로컬 키를 더하지 않고 그대로 상속한다. 즉 이 날짜의 핵심은 키를 많이 늘리는 것이 아니라, 최소 키셋으로 전투/비전투 분기를 세우는 데 있다.
- `BT_Monster_Normal_AssetDump.txt`는 루트가 `Selector`이며, `Target is Set`이면 `BTTask_MonsterTrace -> BTTask_MonsterAttack`, `Target is Not Set`이면 `BTTask_MonsterWait -> BTTask_Patrol`로 갈라지는 구조를 보여 준다. 강의에서 설명한 "감지 기반 전투 브랜치와 비전투 브랜치"가 실제 프로젝트에서도 그대로 굴러간다는 뜻이다.
- `MonsterBase_FileDump.txt`와 `MonsterController_FileDump.txt`를 보면 `MonsterBase`는 `BeginPlay()`에서 `FindMonsterInfo(mDataName)`를 호출하고, 아직 데이터가 없으면 `mMonsterInfoLoadDelegate`에 자신을 묶어 비동기 로드 완료를 기다린다. 또한 컨트롤러는 `SightRadius = 800`, `LoseSightRadius = 800`, `PeripheralVisionAngleDegrees = 180`으로 세팅되고, 감지 결과를 블랙보드 `Target`에 곧바로 써 넣는다. 그래서 이 날의 구조는 "데이터 로드 축"과 "타깃 감지 축"이 시작부터 분리되어 있다는 점이 중요하다.
- 현재 저장소는 이 원형 구조를 지우지 않고, `AMonsterGAS`, `AMonsterGASController`, `UMonsterAttributeSet` 축을 나란히 둔 상태다. 즉 `Pawn 본체 + 감지 + BT + 데이터 로딩`이라는 큰 골격은 유지되고, 런타임 수치 저장소가 `IMonsterState`에서 `AttributeSet` 쪽으로 옮겨 갔다고 보는 편이 정확하다.
- `AMonsterGAS::BeginPlay()`를 보면 `mASC->InitAbilityActorInfo(this, this)`와 `GiveAbility(UGameplayAbility_Attack::StaticClass())`가 추가된다. 즉 `260414`가 만든 "데이터를 읽는 몬스터 본체" 위에 지금은 GAS 진입점까지 붙은 상태다.
- `AMonsterGAS::MonsterInfoLoadComplete()`는 `FMonsterInfo`를 `mAttack`, `mHP`, `mDetectRange` 같은 멤버에 직접 복사하지 않고, `mAttributeSet->SetAttack()`, `SetDefense()`, `SetHP()`, `SetWalkSpeed()`, `SetAttackDistance()`, `SetDetectRange()`로 넘긴다. 그리고 이동 속도와 AI 감지 범위도 다시 `AttributeSet` 값을 읽어 반영한다. 즉 이 날짜의 "데이터 주입" 개념이 현재 branch에서는 `DataTable -> AttributeSet -> AI/Movement`로 한 단계 확장된 셈이다.
...
현재 branch 비교 메모를 붙이면 더 흥미롭다.
원래 강의의 `MonsterState`가 맡던 "공통 전투/이동 수치 저장" 역할은 지금 `UBaseAttributeSet`과 `UMonsterAttributeSet`으로 이어진다.
`UBaseAttributeSet`은 `Attack`, `Defense`, `HP`, `MP`, `WalkSpeed`, `RunSpeed`, `AttackDistance`, `Gold`를 들고, `UMonsterAttributeSet`은 여기에 `DetectRange`를 덧붙인다.
그리고 `AMonsterGAS`는 `UAbilitySystemComponent`와 이 `AttributeSet`을 함께 소유한다.
...
즉 `260414`의 설계 포인트는 사라진 것이 아니다.
"몬스터 객체 안의 float 묶음"이던 공통 상태가, 현재 branch에서는 "ASC가 관리하는 AttributeSet"으로 수납 위치를 바꿔 더 넓은 시스템과 연결된 셈이다.
...
현재 branch를 함께 보면 이 분리가 한 단계 더 진행된다.
`FMonsterInfo`는 여전히 원본 설계도지만, 런타임 상태는 더 이상 `IMonsterState` 멤버에만 머물지 않고 `AttributeSet`으로도 복사된다.
그래서 이후 `BTTask_TraceGAS`, `BTTask_AttackGAS`, `GameplayAbility_Attack`, `GameplayEffect_Damage`가 같은 수치 집합을 공유할 수 있다.
...
현재 `AMonsterGAS::MonsterInfoLoadComplete()`까지 같이 보면 이 장의 설계가 지금 branch에서 어떻게 이어졌는지도 확인할 수 있다.
...
즉 원래 강의의 `MonsterInfoLoadComplete()`가 "테이블 값을 몬스터 멤버로 복사하는 순간"이었다면,
현재 branch의 대응 지점은 "테이블 값을 AttributeSet으로 복사해 AI와 GAS가 함께 쓰는 순간"이다.
그래서 `260414`는 지금 읽어도 여전히 중요하다.
이날 만든 데이터 공급 구조가 있어야 나중 `260422`의 GAS 데미지 루프도 자연스럽게 올라간다.
...
5. 현재 branch에서는 이 수치 저장 계층의 상당 부분이 `AttributeSet / ASC`로 옮겨 가며, 이후 GAS 전투 루프의 기반이 된다.
6. 공식 문서 기준으로는 이 흐름이 `Behavior Tree + Blackboard + AI Perception + Data Driven Gameplay + Asset Management` 조합으로 설명되고, 실제 C++에서는 `MonsterNormal::PossessedBy()`가 BT 실행을 연결하고 `MonsterBase::BeginPlay()`와 `MonsterInfoLoadComplete()`가 데이터 주입과 AI 반응값 적용을 마무리한다.

이 여섯 층이 있어야 이후 강의의 순찰, 추적, 공격, 애니메이션 루프가 자연스럽게 올라갈 수 있다.
...
- `IMonsterState` 중심 저장 구조와 `UBaseAttributeSet / UMonsterAttributeSet` 중심 저장 구조가 어떻게 대응되는지 비교할 수 있는가
