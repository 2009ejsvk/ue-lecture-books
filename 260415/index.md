...
- `MonsterSpawnPoint::SpawnMonster()`, `MonsterBase::SetPatrolPoints()`, `BTTask_Patrol`, `BTTask_MonsterTrace`가 실제 프로젝트에서 어떻게 이어지는지 설명할 수 있다.
...
6. 현재 프로젝트 C++ 코드를 읽으며, 위 구조가 `MonsterSpawnPoint`, `MonsterBase`, `MonsterController`, `BTTask_Patrol`, `BTTask_MonsterTrace` 안에서 어떻게 이어지는지 확인한다.

## 2026-04-23 덤프 반영 메모

이번 보강에서는 `MonsterSpawnPoint`, `MonsterController`, `BTTask_*` 덤프를 기준으로, 스폰과 순찰이 실제로 어떤 데이터 변환을 거쳐 전개되는지 본문 앞에 먼저 고정해 둔다.
...
- 같은 덤프에서 실제 스폰 루프도 드러난다. `SpawnActor<AMonsterBase>(...)`로 몬스터를 만든 뒤 `SetSpawnPoint(this)`와 `SetPatrolPoints(mPatrolPoints)`를 호출하므로, SpawnPoint는 단순 배치 마커가 아니라 "순찰 데이터를 주입하는 시작점"이다.
- `MonsterNormal_FileDump.txt`를 보면 `PossessedBy()`에서 컨트롤러에 `SetAITree(TEXT("/Game/Monster/BT_Monster_Normal.BT_Monster_Normal"))`를 전달한다. 즉 Behavior Tree 연결은 블루프린트 편집기에서 우연히 붙는 것이 아니라, 몬스터가 AI에 빙의되는 시점에 명시적으로 결정된다.
- `MonsterController_FileDump.txt`와 `BTTask_MonsterTrace_FileDump.txt`를 같이 보면, 감지 성공/해제 시 블랙보드 `Target`이 갱신되고, 추적 태스크는 `MoveToActor(Target)`와 함께 애니메이션을 `Run`으로 바꾼다. 그리고 타깃과의 거리가 `AttackDistance` 이하가 되면 일부러 `Failed`를 반환해 다음 공격 브랜치로 넘긴다. 여기서 `Failed`는 오류가 아니라 "추적 종료 신호"에 가깝다.
- `BTTask_Patrol_FileDump.txt`는 순찰 태스크가 `MoveToLocation(Monster->GetPatrolPoint())`와 `Walk` 전환을 담당하고, 목표점과의 거리가 `5.f` 이하가 되면 끝난 뒤 `OnTaskFinished()`에서 `NextPatrol()`을 호출한다는 사실을 보여 준다. 즉 순찰은 스플라인을 실시간으로 따라가는 것이 아니라, 미리 뽑아 둔 점 배열을 하나씩 소비하는 구조다.
...
```cpp
// 데이터 테이블에서 읽은 이동/감지/보상 값을 몬스터 상태에 복사한다.
...
여기에 현재 `MonsterBase`의 순찰 규칙도 같이 보면 더 선명하다.
...
즉 `260415`의 BT 연결은 “PossessedBy 시점에 몬스터별 AI 트리를 실제 실행 상태로 올린다”는 의미가 더 정확하다.
...
3. `MonsterNormal::PossessedBy()`에서 `BT_Monster_Normal`이 실제 실행되는가
...
현재 `BT_Monster_Normal` 덤프를 같이 보면 이 구조가 더 선명하다.
...
### 5.5 `AMonsterBase`: 순찰 데이터와 추적/공격 거리 데이터를 함께 들고 있는 허브

SpawnPoint가 문맥을 넘겨 줘도, 그 데이터를 실제로 보관할 공통 베이스가 필요하다.
그 역할을 `AMonsterBase`가 맡는다.
...
protected:
    ...
    // 실제 순찰 좌표 배열
    TArray<FVector> mPatrolPoints;
    // 현재 목표 순찰점 인덱스. 0이 아닌 1부터 시작하게 되어 있다.
...
- MonsterBase는 그 문맥을 실제 런타임 상태로 보관한다.
- Patrol 태스크는 이 공통 인터페이스만 읽으면 된다.
...
여기에 `MonsterInfoLoadComplete()`로 들어오는 `mWalkSpeed`, `mRunSpeed`, `mDetectRange`, `mAttackDistance`도 함께 보면 더 선명하다.
즉 `MonsterBase`는 단순 순찰 허브가 아니라, 순찰과 추적을 모두 떠받치는 공통 상태 허브다.

### 5.6 `PossessedBy()`와 `SetAITree()`: 스폰된 몬스터를 실제 AI 상태로 올리는 단계

스폰만 됐다고 AI가 바로 생각하는 것은 아니다.
실제로 컨트롤러가 빙의하고 Behavior Tree를 실행해야 한다.
그 연결점이 `AMonsterNormal::PossessedBy()`다.
...
### 5.8 `UBTTask_Patrol`: Target이 없을 때만 `MoveToLocation()`으로 순찰한다
...
### 5.9 `UBTTask_MonsterTrace`: Target이 생기면 `MoveToActor()`로 추적하고, 공격 거리 안에 들어오면 전투 다음 단계로 넘긴다
...
2. `OnConstruction()`이 스플라인을 `PatrolPoints` 배열로 번역한다.
3. `SpawnMonster()`가 몬스터를 만들고 `SpawnPoint`, `PatrolPoints`를 넘긴다.
4. `MonsterNormal::PossessedBy()`와 `SetAITree()`가 BT를 실제 실행 상태로 올린다.
5. `OnTarget()`이 `Target`과 이동 속도를 갱신한다.
6. `BTTask_Patrol`과 `BTTask_MonsterTrace`가 각각 `MoveToLocation()`, `MoveToActor()`로 비전투/전투 진입 이동을 나눠 처리한다.
...
6. 공식 문서 기준으로는 이 흐름이 `Spawn + Navigation + AI Perception + Behavior Tree` 조합으로 설명되고, 실제 C++에서는 `SpawnMonster()`가 SpawnPoint 문맥과 Patrol 문맥을 넘기고 `BTTask_Patrol`과 `BTTask_MonsterTrace`가 그 문맥을 실제 이동 명령으로 바꾼다.
...
- `SpawnMonster() -> SetPatrolPoints() -> BTTask_Patrol` 흐름을 설명할 수 있는가
- `OnTarget() -> Blackboard.Target -> BTTask_MonsterTrace -> MoveToActor()` 흐름을 설명할 수 있는가
...
5. `ClearSpawn()`처럼 준비만 된 훅을 실제 사망 루프와 연결할 때, SpawnPoint와 MonsterBase 중 어느 쪽 책임으로 두는 것이 더 자연스러울까.
...
5. `MonsterSpawnPoint.cpp`, `MonsterBase.h`, `BTTask_Patrol.cpp`, `BTTask_MonsterTrace.cpp`를 나란히 열어 "편집용 스플라인이 런타임 이동으로 바뀌는 경로"를 화살표 다이어그램으로 정리해 본다.
