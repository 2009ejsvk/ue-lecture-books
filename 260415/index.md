...
## 제4장. 현재 프로젝트 C++ 코드로 다시 읽는 260415 핵심 구조

### 4.1 왜 260415는 "몬스터 하나"가 아니라 "필드 문맥"을 만드는 강의인가
...
### 4.2 `AMonsterSpawnPoint`: 스폰 규칙과 순찰 입력을 한 액터에 묶는다
...
### 4.3 `OnConstruction()`: 스플라인을 PatrolPoints 배열로 번역한다
...
### 4.4 `SpawnMonster()`: 몬스터를 만들고 SpawnPoint 문맥과 Patrol 문맥을 함께 전달한다
...
### 4.5 `AMonsterBase`: 순찰 데이터와 추적/공격 거리 데이터를 함께 들고 있는 허브
...
### 4.6 `PossessedBy()`와 `SetAITree()`: 스폰된 몬스터를 실제 AI 상태로 올리는 단계
...
### 4.7 `OnTarget()`: 감지 이벤트가 Target 값과 이동 속도를 동시에 바꾼다
...
### 4.8 `UBTTask_Patrol`: Target이 없을 때만 `MoveToLocation()`으로 순찰한다
...
### 4.9 `UBTTask_MonsterTrace`: Target이 생기면 `MoveToActor()`로 추적하고, 공격 거리 안에 들어오면 전투 다음 단계로 넘긴다
...
6. 실제 C++에서는 `SpawnMonster()`가 SpawnPoint 문맥과 Patrol 문맥을 넘기고, `BTTask_Patrol`과 `BTTask_MonsterTrace`가 그 문맥을 실제 이동 명령으로 바꾼다.
