---
title: 260424 부록 - AcademyUtility 덤프 워크플로
---

# 부록. AcademyUtility 덤프 워크플로

[이전: 260424 고급 1편](../03_advanced_abilitytask_and_montage_ability_playback/) | [허브](../)
...
`260424` 본편이 `MonsterGAS 죽음 처리`, `커스텀 ASC`, `AbilityTask` 같은 설계 확장을 다뤘다면,
이 부록은 그 내용을 나중에 다시 추적하거나 문서로 보강할 때 `어떤 명령으로 무엇을 덤프하고 어떤 파일을 읽으면 되는가`를 정리한다.
...
- `Academy.Dump.Path`가 추가되면서 `/Game/...`, `/Script/...`, 일반 파일 경로를 한 번에 섞어 덤프할 수 있게 됐다.
- `Academy.Generate.Header`, `Academy.Generate.CppDraft`까지 있어서 블루프린트를 읽을 때 구조 초안을 빠르게 얻을 수 있다.
- 즉 이 플러그인은 단순한 파일 출력기가 아니라, 교재 집필용 스냅샷 생성 도구에 가깝다.
...
Academy.Dump.Path "/Script/UE20252.GameplayAbility_Base"
Academy.Dump.Path "/Script/UE20252.MonsterGAS"
Academy.Dump.Path "/Script/UE20252.MonsterAttributeSet"
...
즉 현재 덤프는 블루프린트, C++ 소스, AI 자산, 일반 파일, 태그, 레벨까지 꽤 넓게 확보된 상태다.
...
- `*_GameplayTagsDump.txt`
  현재 프로젝트에 실제 등록된 태그와 리다이렉트를 볼 때 쓴다.
- `*_LevelDump.txt`
  맵에 무엇이 어디 배치됐는지 볼 때 쓴다.
...
- `260414`
  `BT_Monster_Normal`, `BB_Monster_Base`, `DT_MonsterInfo`, `MonsterBase`, `MonsterController`
- `260415`
  `MonsterSpawnPoint`, `BTTask_Patrol`, `BTTask_MonsterTrace`, `BTTask_PatrolGAS`, `BTTask_TraceGAS`
- `260416`
  `MonsterAnimInstance`, `MonsterGASAnimInstance`, `BTTask_MonsterAttack`, `BTTask_AttackGAS`
- `260420`
  `ItemBox`, `MonsterBase`, `MonsterGAS` 사망 후반 처리 관련 클래스
- `260421`
  `PlayerCharacterGAS`, `ShinbiGAS`, `GameplayAbility_Base`, `GameplayEffect_ManaCost`, `BaseAttributeSet`, `PlayerAttributeSet`
- `260422`
  `GameplayAbility_Attack`, `GameplayEffect_Damage`, `DefaultGameplayTags.ini`
- `260423`
  `GameplayCueNotify_StaticBase`, `MonsterGASController`, `MonsterNormalGAS_Warrior`
- `260424`
  `BaseAttributeSet`, `MonsterAttributeSet`, `GameplayEffect_CoolDown`, `PlayerAnimInstance`, `PlayerTemplateAnimInstance`

## 이번 개정에서 추가로 뽑아두면 좋은 경로

`260424` 보강을 기준으로 하면 아래 경로를 추가 확보해 두면 이후 문서 작업이 훨씬 쉬워진다.

```text
/Script/UE20252.BaseAttributeSet
/Script/UE20252.GameplayAbility_Base
/Script/UE20252.GameplayEffect_ManaCost
/Script/UE20252.GameplayEffect_CoolDown
/Script/UE20252.PlayerCharacterGAS
/Script/UE20252.ShinbiGAS
/Script/UE20252.PlayerAnimInstance
/Script/UE20252.PlayerTemplateAnimInstance
/Script/UE20252.MonsterGAS
/Script/UE20252.MonsterAttributeSet
/Script/UE20252.MonsterGASAnimInstance
/Script/UE20252.GameplayAbility_Attack
```
...
1. `260424` 한 챕터를 골라 필요한 타깃만 경로 목록으로 정리하고 `Academy.Dump.Path @파일` 방식으로 다시 덤프해 본다.
2. 같은 대상을 `Academy.Dump.List`와 `Academy.Dump.Path`로 각각 뽑아 보고, 어떤 쪽이 문서 재현성에 더 유리한지 비교해 본다.
3. `DefaultGameplayTags_GameplayTagsDump.txt`와 `Main_LevelDump.txt`를 함께 읽고, 코드/자산만으로는 안 보이는 정보가 무엇인지 따로 메모해 본다.
