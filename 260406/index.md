...
- `Gameplay Framework`, `Setting Up a Character`, `Setting Up a Game Mode`, `Enhanced Input`, `Player Input` 공식 문서가 왜 `260406`과 직접 연결되는지 설명할 수 있다.
...
6. 언리얼 공식 문서를 통해 `Gameplay Framework`, `Character`, `GameMode`, `Enhanced Input` 개념이 엔진 표준 문법으로 어떻게 정리되는지 확인한다.
7. 현재 프로젝트의 실제 C++ 코드를 읽으며, 위 구조가 `PlayerCharacter`, `Shinbi`, `DefaultGameMode`, `MainPlayerController`, `InputData` 안에서 어떻게 정리되었는지 확인한다.

## 2026-04-23 덤프 반영 메모

`Saved\AcademyUtility` 덤프를 다시 확인해 보니, `260406`의 핵심 설명은 현재 프로젝트 기준으로도 그대로 살아 있다.

- `InputData_SourceDump.txt`
  `UDefaultInputData` CDO가 `/Game/Input/IMC_Default`와 `IA_Move`, `IA_Rotation`, `IA_Jump`, `IA_Attack`, `IA_Skill1`를 생성자에서 한 번에 잡는다. 즉 입력 자산 창고라는 설명이 비유가 아니라 실제 구현이다.
- `PlayerCharacter_SourceDump.txt`
  `APlayerCharacter`는 `SpringArm`, `Camera`, `TeamID`를 들고 있고, `InputAttack`, `Skill1`, `NormalAttack`, `Skill1Casting` 같은 가상 훅을 이미 베이스 클래스에 준비해 둔다.
- `Shinbi_SourceDump.txt`, `Wraith_SourceDump.txt`
  두 파생 캐릭터는 같은 베이스를 상속하지만 공격 구현은 서로 다르게 오버라이드한다. `260406`이 “공통부와 개별부를 나누는 날”이라는 해석이 덤프로도 다시 확인된다.
- `MainPlayerController_SourceDump.txt`, `DefaultInput_FileDump.txt`
  컨트롤러는 `bShowMouseCursor = true`, `FInputModeGameAndUI`를 사용하고, 프로젝트 입력 설정은 마우스 민감도 `0.07`로 잡혀 있다. 즉 이 날짜는 단순 이동 입력뿐 아니라 이후 마우스 기반 조작의 바닥도 같이 깔고 있다.
...
public:
    // 이 데이터 자산이 대표로 들고 있는 입력 컨텍스트
    TObjectPtr<UInputMappingContext> mContext;

protected:
    // "Move", "Jump" 같은 이름표로 액션을 찾기 위한 저장소
...
```cpp
// 이 옵션이 켜져 있으면 컨트롤러의 Yaw 회전을 캐릭터 몸이 따라간다.
bUseControllerRotationYaw = true;
```
...
{
    // 실제 공격 내용은 파생 클래스가 오버라이드한 InputAttack으로 넘긴다.
    InputAttack();
}
```
...
## 제4장. 언리얼 공식 문서로 다시 읽는 260406 핵심 구조

### 4.1 왜 260406부터 공식 문서를 같이 보는가

`260406`은 블루프린트 플레이어를 C++ 구조로 옮기는 날이라서, 입문자 입장에서는 "이제부터는 프로젝트 코드만 보면 되나?"라는 느낌이 들기 쉽다.
하지만 실제로는 이 날짜부터 언리얼 공식 문서의 `Gameplay Framework`, `Character`, `GameMode`, `Input` 용어를 같이 보는 편이 훨씬 이해가 빠르다.
...
즉 이 장의 목적은 `APlayerCharacter`, `ADefaultGameMode`, `UDefaultInputData`를 더 복잡하게 설명하는 것이 아니라, 강의에서 쓴 구조가 언리얼 표준 문서에서는 어떤 역할 이름으로 정리되는지 연결하는 데 있다.

### 4.2 공식 문서의 `Gameplay Framework`는 강의의 `PlayerCharacter / GameMode / PlayerController` 분리를 한 번에 묶어 준다

강의 1장과 2장의 핵심은 플레이어 시스템을 하나의 거대한 클래스가 아니라 여러 책임으로 나누는 것이다.
공식 문서의 `Gameplay Framework`도 정확히 같은 틀을 설명한다.

- `Character`: 월드 안에서 움직이고 보이는 플레이어 본체
- `PlayerController`: 입력을 해석하고 조종 문맥을 가지는 주체
- `GameMode`: 어떤 Pawn과 Controller를 기본으로 쓸지 정하는 월드 규칙

즉 `260406`은 단순히 C++ 문법을 배우는 날이 아니라, 언리얼이 원래 기대하는 플레이어 구조에 프로젝트를 맞추는 날이라고 볼 수 있다.
강의에서 `APlayerCharacter`, `AMainPlayerController`, `ADefaultGameMode`를 따로 두는 이유가 공식 문서 기준으로도 아주 정석적이라는 뜻이다.

### 4.3 `Setting Up a Character` 문서는 왜 `Character` 기반 베이스 클래스가 출발점인지 다시 확인시켜 준다

강의는 플레이어를 일반 `Actor`나 빈 `Pawn`이 아니라 `Character` 기반으로 옮긴다.
공식 문서의 `Setting Up a Character`도 이 선택이 왜 자연스러운지 잘 설명한다.

문서 기준으로 `Character`는 이미 아래 전제를 갖춘 클래스다.

- 캡슐 충돌
- 스켈레탈 메시 연결 구조
- 이동 컴포넌트 기반 캐릭터 이동
- 애니메이션 블루프린트와 연결하기 쉬운 구조

즉 `260406`의 `APlayerCharacter`는 "처음부터 다 만드는 클래스"가 아니라, 엔진이 이미 준비해 둔 플레이어용 뼈대 위에 우리 프로젝트 규칙을 얹는 클래스다.
이 관점을 잡고 보면 생성자에서 `SpringArm`, `Camera`, 회전 옵션, 점프 옵션을 붙이는 코드도 블루프린트 세팅을 C++로 옮긴다는 의미가 더 선명해진다.

### 4.4 공식 문서의 `Game Mode`와 `Enhanced Input`는 `진입점`과 `입력 파이프라인`을 분리해서 읽게 만든다

강의 2장과 3장은 사실 서로 다른 두 층을 다룬다.

- `GameMode`: 누가 기본 플레이어인지 정하는 진입점
- `InputData / Enhanced Input`: 입력 자산을 모으고 함수와 연결하는 파이프라인

공식 문서의 `Setting Up a Game Mode`는 `DefaultPawnClass`, `PlayerControllerClass` 같은 값이 왜 월드 규칙인지 설명해 준다.
반면 `Enhanced Input`과 `Player Input` 문서는 `Mapping Context`, `Input Action`, 액션 바인딩이 왜 별도 자산과 등록 단계로 분리되는지 보여 준다.

이 문서 흐름으로 다시 보면 `260406`의 핵심도 더 분명해진다.

- `ADefaultGameMode`는 플레이어 입장권을 정한다.
- `UDefaultInputData`는 입력 자산 창고 역할을 한다.
- `BeginPlay()`는 컨텍스트를 등록한다.
- `SetupPlayerInputComponent()`는 실제 함수를 연결한다.

즉 강의가 일부러 `GameMode`와 `InputData`를 같은 날짜에 묶은 이유는 "플레이어가 월드에 들어오는 길"과 "들어온 뒤 조작되는 길"을 함께 완성하기 위해서다.

### 4.5 260406 공식 문서 추천 읽기 순서

이번 날짜는 아래 순서로 공식 문서를 읽으면 가장 자연스럽다.

1. [Gameplay Framework in Unreal Engine](https://dev.epicgames.com/documentation/en-us/unreal-engine/gameplay-framework-in-unreal-engine)
...
3. [Setting Up a Game Mode in Unreal Engine](https://dev.epicgames.com/documentation/en-us/unreal-engine/setting-up-a-game-mode-in-unreal-engine)
4. [Enhanced Input in Unreal Engine](https://dev.epicgames.com/documentation/en-us/unreal-engine/enhanced-input-in-unreal-engine)
5. [Quick Start Guide to Player Input in Unreal Engine C++](https://dev.epicgames.com/documentation/en-us/unreal-engine/quick-start-guide-to-player-input-in-unreal-engine-cpp?application_version=5.6)
6. [Implementing your Character in Unreal Engine](https://dev.epicgames.com/documentation/en-us/unreal-engine/implementing-your-character-in-unreal-engine)

이 순서가 좋은 이유는 먼저 `누가 무엇을 맡는가`를 프레임워크 관점에서 잡고, 그 다음 `Character`, `GameMode`, `Input`을 각각 세부로 내려가 읽을 수 있기 때문이다.

### 4.6 장 정리

공식 문서 기준으로 다시 보면 `260406`은 아래 다섯 가지를 배우는 날이다.

1. 플레이어 시스템은 `Character`, `Controller`, `GameMode`로 책임이 나뉜다.
2. `Character`는 플레이어 본체를 만드는 가장 표준적인 출발점이다.
3. `GameMode`는 어떤 Pawn과 Controller를 기본으로 쓸지 정한다.
4. `Enhanced Input`은 입력 자산과 실행 함수를 분리하는 표준 입력 체계다.
5. 그래서 `260406`의 C++ 전환은 단순 번역이 아니라, 프로젝트 구조를 언리얼 표준 틀에 맞추는 과정이다.

---

## 제5장. 현재 프로젝트 C++ 코드로 다시 읽는 260406 핵심 구조

### 5.1 왜 260406부터는 "실제 소스 읽기"가 특히 중요해지는가
...
### 5.2 `APlayerCharacter`: 플레이어 공통부가 어디까지인지 헤더만 봐도 보인다
...
### 5.3 `APlayerCharacter` 생성자: 블루프린트에서 손으로 하던 세팅이 코드로 굳어지는 순간
...
### 5.4 `AShinbi`: 공통 베이스 위에 외형과 개별 동작을 올리는 파생 클래스
...
### 5.5 `ADefaultGameMode`와 `AMainPlayerController`: 누가 기본 플레이어이고, 누가 조종하는가
...
### 5.6 `UDefaultInputData`: 입력 자산을 플레이어 클래스 밖으로 빼는 설계
...
public:
    // 이 입력 데이터가 대표로 들고 있는 매핑 컨텍스트
    TObjectPtr<UInputMappingContext> mContext;

protected:
    // 문자열 키로 액션을 찾기 위한 저장소
    TMap<FString, TObjectPtr<UInputAction>> mActions;
...
{
    // 먼저 플레이어가 기본으로 사용할 IMC_Default를 찾는다.
    static ConstructorHelpers::FObjectFinder<UInputMappingContext> InputContext(
        TEXT("/Script/EnhancedInput.InputMappingContext'/Game/Input/IMC_Default.IMC_Default'")
    );
    if (InputContext.Succeeded())
    ...
    // 공격 액션도 미리 넣어 두면 플레이어 코드는 경로를 몰라도 된다.
...
### 5.7 `BeginPlay`와 `SetupPlayerInputComponent`: 등록과 바인딩은 역할이 다르다
...
void APlayerCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
    ...
    // 액션 기반 입력을 쓰기 위해 EnhancedInputComponent로 바꿔 잡는다.
    TObjectPtr<UEnhancedInputComponent> Input =
        Cast<UEnhancedInputComponent>(PlayerInputComponent);

    if (IsValid(Input))
    {
        // 입력 경로를 직접 적지 않도록 데이터 자산에서 액션을 가져온다.
        const UDefaultInputData* InputData = GetDefault<UDefaultInputData>();
...
### 5.8 `RotationKey`, `JumpKey`, `AttackKey`: 기본 조작 루프와 확장 훅이 동시에 보이는 지점
...
### 5.9 `AShinbi::InputAttack()`: 베이스 클래스가 진입점만 주고, 파생 클래스가 내용을 채운다는 뜻
...
그리고 이번 정리본에서는 여기에 더해, 언리얼 공식 문서 기준으로 `Gameplay Framework`, `Character`, `GameMode`, `Enhanced Input`이 어떤 표준 용어로 설명되는지도 먼저 확인하고, 그 구조가 현재 `UE20252` 소스에서 `PlayerCharacter`, `Shinbi`, `DefaultGameMode`, `MainPlayerController`, `InputData`로 어떻게 정리되어 있는지도 함께 읽는다.
...
- `Gameplay Framework`, `Setting Up a Character`, `Setting Up a Game Mode`, `Enhanced Input` 중 어떤 문서가 각 파트를 보강하는지 연결할 수 있는가?
...
4. 공식 문서의 `Gameplay Framework` 그림을 기준으로 보면, `GameMode`, `PlayerController`, `PlayerCharacter` 분리는 어떤 면에서 더 당연하게 보이는가?
5. `GameMode`, `PlayerController`, `PlayerCharacter`를 분리하는 구조는 "플레이어 하나가 다 한다"는 방식보다 어떤 장점을 주는가?
...
4. 공식 문서의 `Gameplay Framework`, `Setting Up a Character`, `Enhanced Input`를 읽고, 강의 용어와 공식 용어가 어떻게 대응되는지 표로 정리해 본다.
5. `APlayerCharacter`, `UDefaultInputData`, `ADefaultGameMode`, `AMainPlayerController`를 보고 "공통 베이스 -> 입력 자산 -> 월드 진입점 -> 조종 주체" 흐름을 화살표로 정리해 본다.
