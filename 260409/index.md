...
- `Collision Overview`, `OnHit`, `Trace`, `Niagara`, `Audio` 공식 문서가 왜 `260409`와 직접 연결되는지 설명할 수 있다.
...
5. 언리얼 공식 문서를 통해 충돌 응답, 트레이스, 히트 이벤트, VFX, 오디오가 엔진 표준 용어로 어떻게 정리되는지 확인한다.
6. 현재 프로젝트 C++ 코드를 읽으며, 위 구조가 `Shinbi`, `MonsterBase`, `ProjectileBase`, `WraithBullet` 안에서 어떻게 하나의 전투 파이프라인으로 이어지는지 확인한다.

## 2026-04-23 덤프 반영 메모

`260409`에 대응하는 현재 덤프는 “공격 결과물”이 생각보다 한 액터 안에 촘촘히 묶여 있다는 점을 잘 보여 준다.

- `WraithBullet_SourceDump.txt`
  `AWraithBullet`은 `mParticle`, `mHitParticle`, `mHitSound`, `mHitDecal`, `mAttack`, `mOwnerController`를 함께 들고 있다. 즉 원거리 공격은 발사체 하나가 데미지와 피드백을 같이 완성하는 구조다.
- 같은 덤프
  `BulletHit()`는 `TakeDamage()` 호출 뒤에 히트 파티클, 사운드, 데칼을 순서대로 생성한다. “맞았다는 결과를 하나의 파이프라인으로 본다”는 이 문서의 핵심과 정확히 맞물린다.
- `DecalBase_SourceDump.txt`
  `ADecalBase`는 루트가 `UDecalComponent` 하나인 아주 얇은 래퍼이고 `SetDecalMaterial()`만 노출한다. 그래서 데칼은 거대한 이펙트 시스템이 아니라, 표면 정보 전용 액터로 분리돼 있다.
- `Wraith_SourceDump.txt`
  `AWraith`가 직접 데미지를 계산하는 대신 투사체 클래스를 포함해 공격 책임을 넘긴다는 점도 덤프로 다시 확인된다.
...
```cpp
// 플레이어 본체 캡슐은 Player 프로파일을 쓴다.
GetCapsuleComponent()->SetCollisionProfileName(TEXT("Player"));
...
// 플레이어 공격용 탄환/충돌체는 PlayerAttack 프로파일을 쓴다.
...
```cpp
// 시작점에서 끝점까지 캡슐을 쓸어 가며 맞은 대상을 전부 모은다.
    ...
    AActor* DamageCauser)
{
    ...
    // 현재는 실제 반영보다 디버그 메시지로 피해량 확인에 초점을 둔다.
...
## 제4장. 언리얼 공식 문서로 다시 읽는 260409 핵심 구조

### 4.1 왜 260409부터 공식 문서를 같이 보는가

`260409`은 공격 판정, 데미지, 이펙트, 사운드, 투사체를 한 번에 묶는 날이라서, 입문자 입장에서는 기능이 갑자기 너무 많아진 것처럼 보이기 쉽다.
하지만 공식 문서 기준으로 보면 이들은 전부 별개의 기능이 아니라 `Collision`, `Trace`, `Damage`, `VFX`, `Audio`라는 엔진 표준 층으로 정리된다.
...
- [Using the OnHit Event](https://dev.epicgames.com/documentation/en-us/unreal-engine/using-the-onhit-event)
- [Using a Single Line Trace (Raycast) by Channel in Unreal Engine](https://dev.epicgames.com/documentation/en-us/unreal-engine/using-a-single-line-trace-raycast-by-channel-in-unreal-engine)
- [UKismetSystemLibrary::SphereTraceSingleByProfile](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/Kismet/UKismetSystemLibrary/SphereTraceSingleByProfile)
- [Creating Visual Effects in Niagara for Unreal Engine](https://dev.epicgames.com/documentation/unreal-engine/creating-visual-effects-in-niagara-for-unreal-engine)
- [Working with Audio in Unreal Engine](https://dev.epicgames.com/documentation/unreal-engine/working-with-audio-in-unreal-engine)
- [Audio Component](https://dev.epicgames.com/documentation/en-us/unreal-engine/audio-component)

즉 이 장의 목적은 기능 수를 더 늘리는 것이 아니라, `260409`에서 나온 전투 결과 처리들이 공식 문서에서는 어떤 시스템 층으로 나뉘는지 보여 주는 데 있다.

### 4.2 공식 문서의 `Collision`, `Trace`, `OnHit` 설명은 강의의 `Sweep / TakeDamage` 구조를 더 선명하게 만든다

강의 2장과 3장의 핵심은 "공격이 일어났다"는 사실을 실제 판정으로 바꾸는 과정이다.
공식 문서 기준으로 이 파이프라인은 크게 세 층으로 나뉜다.

- `Collision Overview`: 누가 누구와 반응할지 규칙을 정한다.
- `Trace` 문서: 실제로 무엇을 맞췄는지 쿼리한다.
- `OnHit` 문서: 충돌이 발생한 순간 후속 로직과 데미지를 연결한다.

이 관점으로 보면 강의의 `SweepMultiByChannel`도 훨씬 덜 추상적으로 읽힌다.
즉 `Shinbi`의 근접 공격은 상시 충돌체를 켜 두는 방식이 아니라, 특정 프레임에 짧은 구간을 쓸어 보면서 누가 맞았는지 판정하는 트레이스/스윕 계열 문법이다.
그리고 실제 탄환 쪽은 `OnComponentHit` 또는 `OnProjectileStop` 같은 히트 이벤트 입구를 사용한다.

즉 `260409`의 본질은 "때린다"가 아니라, `충돌 규칙 -> 판정 쿼리 -> 데미지 전달`을 한 파이프라인으로 만드는 데 있다.

### 4.3 공식 문서의 `Niagara`와 `Audio` 설명은 강의의 히트 피드백을 별도 시스템으로 보게 만든다

입문자는 보통 파티클과 사운드를 "마지막 장식"으로 생각하기 쉽다.
하지만 공식 문서의 `Creating Visual Effects in Niagara`, `Working with Audio`, `Audio Component`를 보면, 언리얼은 시각 피드백과 청각 피드백도 별도 시스템으로 다룬다.

즉 공격 결과는 아래처럼 층이 갈린다.

- 게임플레이 층: 누가 맞았고 얼마의 데미지를 받았는가
- VFX 층: 어디서 어떤 타격 이펙트가 보이는가
- Audio 층: 어디서 어떤 히트 사운드가 들리는가
- Decal/Projectile 층: 충돌 흔적과 원거리 결과를 어떻게 남기는가

이 관점으로 보면 강의에서 `SpawnEmitterAtLocation`, `SpawnSoundAtLocation`, `SpawnDecalAtLocation`를 따로 설명한 이유도 분명해진다.
전부 "맞았다"는 같은 사건에 연결되지만, 실제로는 서로 다른 시스템 층이 동시에 반응하는 것이다.

### 4.4 260409 공식 문서 추천 읽기 순서
...
2. [Using a Single Line Trace (Raycast) by Channel in Unreal Engine](https://dev.epicgames.com/documentation/en-us/unreal-engine/using-a-single-line-trace-raycast-by-channel-in-unreal-engine)
3. [UKismetSystemLibrary::SphereTraceSingleByProfile](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/Kismet/UKismetSystemLibrary/SphereTraceSingleByProfile)
4. [Using the OnHit Event](https://dev.epicgames.com/documentation/en-us/unreal-engine/using-the-onhit-event)
...
이 순서가 좋은 이유는 먼저 `무엇이 맞는가`를 충돌과 트레이스로 이해하고, 그 다음 `맞았을 때 어떤 로직을 실행하는가`를 `OnHit`로 보고, 마지막에 `그 결과를 어떻게 보여 주는가`를 VFX/오디오 문서로 내려갈 수 있기 때문이다.

### 4.5 장 정리

공식 문서 기준으로 다시 보면 `260409`은 아래 다섯 가지를 배우는 날이다.

1. 충돌 규칙이 먼저 있고, 그 위에 판정 쿼리가 올라간다.
2. 근접 공격의 `Sweep`과 원거리 공격의 `Hit`/`Projectile Stop`은 서로 다른 판정 입구다.
3. 데미지는 판정 결과를 게임플레이 상태 변화로 바꾸는 층이다.
4. 파티클과 사운드는 장식이 아니라 별도 피드백 시스템이다.
5. 그래서 `260409`은 여러 기능 나열이 아니라, "맞았다는 결과를 완성하는 엔진 파이프라인"을 배우는 날이다.

---

## 제5장. 현재 프로젝트 C++ 코드로 다시 읽는 260409 핵심 구조

### 5.1 왜 260409은 "공격 기능 모음"이 아니라 하나의 파이프라인으로 읽어야 하는가
...
### 5.2 `UAnimNotify_PlayerAttack`: 애니메이션이 실제 공격 함수를 여는 첫 관문
...
### 5.3 `AShinbi::NormalAttack()`: 근접 전투는 Sweep으로 판정하고 바로 데미지와 피드백을 붙인다
...
### 5.4 `TakeDamage()`: 공격한 쪽이 아니라 맞은 쪽에서 전투 결과가 완성된다
...
### 5.5 충돌 프로파일과 팀 ID: 누가 누구를 때릴 수 있는지 프로젝트 규칙으로 미리 정해 둔다
...
### 5.6 `AWraith::NormalAttack()`과 `AWraithBullet`: 원거리는 판정을 투사체에 위임한다
...
### 5.7 `260409`의 핵심은 "공격하는 코드"보다 "맞았다는 결과를 어떻게 완성하느냐"에 있다
...
6. 언리얼 공식 문서 기준으로는 이 흐름이 `Collision + Trace + OnHit + Niagara + Audio` 층으로 나뉘고, 실제 C++에서는 `Shinbi::NormalAttack()`이 현재 가장 완성된 근접 파이프라인이고 `WraithBullet`은 히트 이펙트 중심의 원거리 파이프라인을 먼저 담당한다.
...
- `Collision Overview`, `OnHit`, `Trace`, `Niagara`, `Audio` 문서가 어느 파트를 보강하는지 연결할 수 있는가
...
4. 공식 문서의 `Collision`, `Trace`, `OnHit` 분류를 먼저 알고 강의를 읽으면 어떤 파트가 더 덜 헷갈릴까
...
4. 공식 문서의 `Collision Overview`, `OnHit`, `Niagara`, `Audio`를 읽고 강의의 어느 예제가 어느 시스템 층에 속하는지 표로 정리해 본다.
